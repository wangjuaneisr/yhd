...
(function($, DateCore, dateLanguage) {
	...
	Calendar.prototype.inputReady = function() {
		var me = this;
		me.$element.attr('readonly', 'readonly');
		me.$element.bind('click', function() {
			...
			if(me.options.initFun && typeof (me.options.initFun) === 'function'){
				me.options.initFun();
	...
	Calendar.prototype.calendarShow = function(mode) {
		...
		var startInput = inputDom.eq(0), endInput = 0, boxTop = startInput
				.offset().top
				+ startInput.outerHeight(), boxLeft = startInput.offset().left, dateboxFrame = evalJquery('<div class="'
				+ me.options.rootNode + '"></div>');
		// boxHtml = evalDom('<div class="' + me.options.rootNode + '"
		...
		// $(inputDom).last().after(dateboxFrame); nicky

		$(inputDom).parent('.timeSelectBar').next('.timeHideModel').show()
				.find('.detailDateCont').append(dateboxFrame);
		...
		var curYear = (initdate && initdate[0]) || DateCore.currentDate.year, curMonth = (initdate && initdate[1])
				|| DateCore.currentDate.month, curDate = (initdate && initdate[2])
				|| DateCore.currentDate.date;
		var changedYear = curYear, changedMonth = curMonth, changedDate = curDate;
		if (startInput.val() !== '') {
		...
		switch (memode) {
		case 'date':
			loadDate.call(me, dateboxFrame, DateCore, curYear, curMonth,
					curDate);
			break;
		case 'month':
			...
			$(document)
					.bind(
							...
							function(event) {
								...
								if (elem
										.closest('.' + me.options.rootNode + '').length === 0
										&& elem.closest(me.$element.selector).length === 0
										&& elem
												.closest('*[data-trigger = "spcalendar"]').length === 0) {
									me.calendarClose();
								}
							});
		}
		...
		dateboxFrame.delegate('td.year', 'click', function(event) {
			...
			loadDate.call(me, dateboxFrame, DateCore, changedYear,
					changedMonth, changedDate);
		});
		...
		dateboxFrame.delegate('td.month', 'click', function(event) {
			...
			if (memode !== 'month') {
				...
				var str1 = splitStr.join('-');
				var str2 = getLastDay(str1);
				var str3 = [];
				str3.push(str1);
				str3.push(str2);
				myselect($(this), str3);
		...
		function myselect(_this, pushresult) {
			if (me.options.radio) {
				...
				arr = str.split(",");
				if (arr.length >= 2) {
					startInput.val(arr[0]);
					endInput.val(arr[1]);// add by zgl
				} else {
					startInput.val(pushresult);
					endInput.val(pushresult);// add by zgl
				}

				(me.options.callbackFun && typeof (me.options.callbackFun) === 'function')
						&& me.options.callbackFun(pushresult);
				...
				if (timeBlock.length >= 2) {
					...
					var mDate = getDateCompared(tempArray.length === 0 ? timeBlock
							: tempArray);
					...
					timeBlock.length = 0;
					(me.options.callbackFun && typeof (me.options.callbackFun) === 'function')
							&& me.options.callbackFun([ mDate.min, mDate.max ]);
					if (me.options.autoclose) {
						me.calendarClose();
					}
				}
	...
	Calendar.prototype.calendarClose = function() {
		...
		$(document).unbind('click.clearDom');
		// return calendarTemp.empty().remove(); nicky
		function abc() {
			calendarTemp.empty().remove();
			$('.timeHideModel').hide();
		}
	...
	function loadDate(dateboxFrame, DateCore, year, month, date) {
		...
		for ( var i = 0; i < me.options.daypanel; i++) {
			...
			generateDateTemplate.call(me, calenderInner, DateCore,
					mutipanel.year, mutipanel.month++, mutipanel.date);
		}
		...
		if (me.$element.eq(0).val() !== '') {
			completionAreaStyle.call(me, me.$element.eq(0).val(),
					me.options.radio ? me.$element.eq(0).val() : me.$element
							.eq(1).val());
		}
		...
		// + '');
		var dateBoxDOM = me.$element.parent('.timeSelectBar').next(
				'.timeHideModel').find('.detailDateCont'); // nicky
	...
	function generateDateTemplate(templateArray, DateCore, year, month, date) {
		...
		templateArray
				.push('<table class="table-condensed table-condensed-date">');
		templateArray
				.push('<thead><tr><th colspan="7" class="switch"><span class="switch-year">'
						+ year
						+ me.$Lang.str_year
						+ '</span>&nbsp;&nbsp;<span class="switch-month">'
						+ me.$Lang.monthsShort[month - 1]
						+ me.$Lang.str_month
						+ '</span></th></tr>');
		...
		for ( var m = 0, k = 0; m < data_Date.length / 7; m++) {
			...
			for ( var n = 0; n < 7; n++) {
				...
				var tempString = year + '-' + autoCompletion(month) + '-'
						+ autoCompletion(data_Date[k].date);
				if (reg.test(data_Date[k].modal)) {
					tempString = '';
				}
				templateArray.push('<td class="day ' + data_Date[k].modal
						+ '" title="' + tempString + '"><p>'
						+ data_Date[k].date + '</p><p class="self-define">'
						+ data_Date[k].spdate + '</p><span></span></td>');
				k++;
			}
			templateArray.push('</tr>');
		}
	...
	function getCellDate(cellObj) {
		if (cellObj.hasClass('old') || cellObj.hasClass('new')
				|| cellObj.hasClass('disabled')) {
			return false;
		}
		return cellObj.attr('title');
	}
	...
	function generateYearTemplate(templateArray, DateCore, year) {
		...
		templateArray
				.push('<thead><tr><th colspan="4" class="switch"><span class="switch-year">'
						+ data_Year[1].year
						+ '--'
						+ data_Year[data_Year.length - 2].year
						+ '</span></th></tr>');
		templateArray.push('</thead>');
		templateArray.push('<tbody>');
		for ( var m = 0, k = 0; m < data_Year.length / 4; m++) {
			...
			for ( var n = 0; n < 4; n++) {
				...
				templateArray.push('<td class="year ' + data_Year[k].modal
						+ '" title="' + tempString + '">' + data_Year[k].year
						+ '</td>');
				k++;
			}
			templateArray.push('</tr>');
	...
	function generateMonthTemplate(templateArray, DateCore, year, month) {
		...
		templateArray
				.push('<table class="table-condensed table-condensed-month">'); // nicky
		templateArray
				.push('<thead><tr><th colspan="6" class="switch"><span class="switch-year">'
						+ year + me.$Lang.str_year + '</span></th></tr>');
		templateArray.push('</thead>');
		templateArray.push('<tbody>');
		for ( var m = 0, k = 0; m < data_Month.length / 6; m++) {
			templateArray.push('<tr>');
			for ( var n = 0; n < 6; n++) {
				var tempString = year + '-'
						+ autoCompletion(data_Month[k].value + 1)+"-01";
				templateArray.push('<td width="16.666%" class="month '
						+ data_Month[k].modal + '" title="' + tempString + '">'
						+ me.$Lang.months[data_Month[k].value] + '</td>');
				k++;
			}
			templateArray.push('</tr>');
		}
	...
	function generateWeekTemplate(templateArray, DateCore, year) {
		...
		templateArray
				.push('<table class="table-condensed table-condensed-week">');
		templateArray
				.push('<thead><tr><th colspan="18" class="switch"><span class="switch-year">'
						+ year + me.$Lang.str_year + '</span></th></tr>');
		templateArray.push('</thead>');
		templateArray.push('<tbody>');
		for ( var x = 0, z = 0, p = 0; x < 3; x++) {
			...
			for ( var o = 0; o < 18; o++, p++) {
				if (p >= data_Week.length)
					break;
				templateArray.push('<td title="' + data_Week[p].weekdateStart
						+ '~' + data_Week[p].weekdateEnd + '" class="week '
						+ data_Week[p].modal + '">' + (p + 1) + '</td>')
			}
			templateArray.push('</tr>');
		}
		templateArray.push('</tbody>');
	...
	function completionAreaStyle(min, max) {
		...
		$('.table-condensed td').each(function() {
			var t = dateToNumber($(this).attr('title'));
			if (isNaN(t)) {
				return;
			}
			if (t >= min && t <= max) {
	...
	function getLastDay(d) {
		...
		return date.getFullYear() + "-" + autoCompletion(date.getMonth() + 1)
				+ "-" + autoCompletion(date.getDate());
	}
