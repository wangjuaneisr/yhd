...
(function(window) {
	...
	DateCore.prototype.Datepanel = function(year, month, date) {
		...
		for ( var i = firstDay + weekStart - 1, startDate = 1; i < dateArray.length; i++) {
			...
			if (me.currentDate.year === year && me.currentDate.month === month
					&& !loop && me.currentDate.date === startDate - 1) {
			...
			curDate = dateToNumber(year + '-' + autoCompletion(month) + '-'
					+ autoCompletion(dateArray[i].date));
			...
			if ((startLimitDate && endLimitDate)
					&& (curDate < startLimitDate && curDate > endLimitDate)) {
				dateArray[i].modal += ' disabled';
			}
		}
		for ( var j = firstDay + weekStart - 2, startDate = monthDays(year,
				(month - 2) < 0 ? 11 : (month - 2)); j >= 0; j--) {
			dateArray[j] = {
				'date' : startDate--,
				'modal' : 'old',
				'spdate' : ''
	...
	DateCore.prototype.Monthpanel = function(year, num) {
		...
		for ( var i = 0; i < monthNum; i++) {
			...
			monthArray[i] = {
			 ...
			 curDate = dateToNumber(year + '-' + autoCompletion(i+1) + '-'
					+ autoCompletion(1));
			 //alert(curDate);


			if ((startLimitDate && !endLimitDate) && (curDate > dateToNumber(startLimitDate))) {
				monthArray[i].modal += ' disabled';
			}

			if ((endLimitDate && !startLimitDate) && (curDate < dateToNumber(endLimitDate))) {
				monthArray[i].modal += ' disabled';
			}

			if ((startLimitDate && endLimitDate)
					&& (curDate < dateToNumber(startLimitDate) && curDate > dateToNumber(endLimitDate))) {
				monthArray[i].modal += ' disabled';
			}
//			if (i == 5) {
//				monthArray[i].modal += ' disabled';
//			}
	...
	DateCore.prototype.Weekpanel = function(year) {
		...
		for ( var j = 0; j < 53; j++) {
			...
			for ( var k = 0; k < 7; k++) {
				...
				if ((startLimitDate && !endLimitDate)
						&& (tempdateNumber > startLimitDate)) {
					weekArray[j].modal = ' disabled';
				}

				if ((endLimitDate && !startLimitDate)
						&& (tempdateNumber < endLimitDate)) {
					weekArray[j].modal = ' disabled';
				}

				if ((startLimitDate && endLimitDate)
						&& (tempdateNumber < startLimitDate && tempdateNumber > endLimitDate)) {
					weekArray[j].modal = ' disabled';
				}
				tempdate = nextDate(tempdate);
				tempdateNumber = dateToNumber(tempdate);
			}
			if (dateToNumber(tempdate) > dateToNumber(year + '-12-31'))
				break;
		}
		return weekArray;
	};
	...
	function hasLimitExec(limitexec) {
		...
		if (limitexec === '') {
			var today = this.currentDate.year + '-'
					+ autoCompletion(this.currentDate.month) + '-'
					+ autoCompletion(this.currentDate.date);
