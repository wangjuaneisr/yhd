$(function(){
	...
	shopHover();//表格hover效果WANGJUAN-促销管理页面
	//chooseCheckBox();//多选框选中标志-自营（创建主题活动主题）

});
function shopHover() {
	$(".hover-shop").hover(function(){
...
function showTable() {
 ...
 function tabChange() {
 	var tab_tit= $('.tab-tit > li');
 		//tab_con = tab_tit.parents('.tab-tit').next('.tab-con-box');//修改指针位置，适用不同的click事件
 	tab_tit.click(function(){
 		var index = $(this).index();
 		tab_con=$(this).parents('.tab-tit').next('.tab-con-box');
 		tab_con.show();
		tab_con.find('.tab-con').eq(index).show().siblings().hide();//console.log(tab_con.find('.tab-con').eq(index));
